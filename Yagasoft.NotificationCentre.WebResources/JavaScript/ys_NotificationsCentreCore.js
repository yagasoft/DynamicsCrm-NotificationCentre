...
async function InitNotificationsMenu()
{
	try
	{
		...
		const libraries = ['ldv_NotificationsCentreLibsJs'];

		if (!window.IsCommonGenericLibraryLoaded)
		{
			libraries.push('ldv_CommonGenericJs');
		}

		await LoadWebResourcesAsync(libraries);
		...
		LoadWebResourceCss('ldv_NotificationsCentreCss');
	}
	catch (e)
	{
...
async function FetchConfiguration()
{
	try
	{
		...
		let config = (Search(userConfig, 'ldv_isdefault', false, 1, true) ?? [])[0] ?? {};
	...
	try
	{
		...
		GetMessagesQuick(async function(messages)
		{
			try
			{
				...
				{
					NcPopupUnreadThreshold = MaxDate([NcLatestMenuOpenDate, NcUnreadThreshold, NcPopupUnreadThreshold,
...
var NcMenu = function()
{
	...
	function openQuickCreate()
	{
		...
		Xrm.Navigation.openForm({ entityName: 'ldv_notificationmessage', useQuickCreateForm: true });
...
async function NcUpdateCache()
{
	...
	NcLatestMenuOpenDate = MaxDate([NcLatestMenuOpenDate, latestMenuOpenDate]);
	NcUnreadThreshold = MaxDate([NcUnreadThreshold, unreadThreshold]);

	NcReadMessages = MergeArrays(NcParseReadCacheRaw(cache.ys_readmessagescache), NcReadMessages);
}
